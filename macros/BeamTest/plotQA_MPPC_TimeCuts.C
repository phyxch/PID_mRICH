...
void plotQA_MPPC_TimeCuts(const int runID = 649)
{
  ...
  for(int i_pixel_x = 0; i_pixel_x < NumOfPixel; ++i_pixel_x)
  {
    for(int i_pixel_y = 0; i_pixel_y < NumOfPixel; ++i_pixel_y)
    {
      ...
      if(runID == 697) counts_sig = h_mTDC[i_pixel_x][i_pixel_y]->Integral(tdc_Start,tdc_Stop);
  ...
  TF1 *f_gaus_OffRing = new TF1("f_gaus_OffRing","gaus",0,5000);
  f_gaus_OffRing->SetParameter(0,100);
  f_gaus_OffRing->SetParameter(1,0.5*(tdc_Start+tdc_Stop));
  f_gaus_OffRing->SetParameter(2,0.5*(tdc_Stop-tdc_Start));
  f_gaus_OffRing->SetRange(tdc_Start,tdc_Stop);
  h_mTDC[x_OffRing][y_OffRing]->Fit(f_gaus_OffRing,"MNQR");
  PlotLine(mean_tdc_Start,mean_tdc_Start,0,0.5*yields,4,2,2);
  PlotLine(mean_tdc_Stop,mean_tdc_Stop,0,0.5*yields,4,2,2);
  ...
  string c_timecut = Form("../../figures/BeamTest_QA/c_TimeCuts_MPPC_%d.eps",runID);
