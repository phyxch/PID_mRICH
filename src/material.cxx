...
material::material()
{
  ...
  const double QE[45]= {        // refer to ../QuantunEfficiency/plot_parameter.C
  ...
  const double lambda[45]={     // refer to ../QuantunEfficiency/plot_parameter.C
  ...
  for(int i=0; i<45; i++){
    std::pair<double, double> tmp (lambda[i], QE[i]);
    QE_Ping.push_back(tmp);
  }
...
double material::extrapQE(double wl)
{
  double QE = -999.;
  
  int iBin = -1;
  for(unsigned int i=0; i<QE_Ping.size()-1; i++){
    if(wl>=QE_Ping.at(i).first && wl<QE_Ping.at(i+1).first){
      iBin = i;
      break;
    }
  }
  if(iBin>=0 && iBin<QE_Ping.size()){
    double x1 = QE_Ping.at(iBin).first;
    double x2 = QE_Ping.at(iBin+1).first;
    double y1 = QE_Ping.at(iBin).second;
    double y2 = QE_Ping.at(iBin+1).second;
    QE = y1 + (wl-x1)*(y2-y1)/(x2-x1);
  }
  if(QE>0.) return QE/100.;
  else return 0;
